...
`;
  ...
  describe("transformArrayAccesses", () => {
    it("transforms simple array access", () => {
      ...
      const bindings = [
        { group: 0, binding: 0, name: "input", access: "read" as const, type: "f32" },
        { group: 0, binding: 1, name: "output", access: "read_write" as const, type: "f32" },
      ];
...
`,
      ...
      expect(result.code).toContain("@group(1) @binding(0) var<uniform> scan_offsets: ScanOffsets");
  ...
  describe("createAllIterationsOffsetsBuffer", () => {
    it("creates correctly aligned offset buffer", () => {
      ...
      const { buffer, alignment } = createAllIterationsOffsetsBuffer(
        numX, numY, length, xsElemStrides, ysElemStrides, minAlignment
      );
