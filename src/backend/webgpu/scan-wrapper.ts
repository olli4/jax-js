...
function getOffsetBindings(
  ...
  const ysBindings = outputs.slice(scanInfo.numCarry, scanInfo.numCarry + scanInfo.numY);
...
export function wrapRoutineForScan(
  ...
  if (offsetDecls) {
    newCode = newCode.substring(0, mainStart) + "\n" + offsetDecls + "\n" + newCode.substring(mainStart);
  }
...
export function createAllIterationsOffsetsBuffer(
  ...
  for (let iter = 0; iter < length; iter++) {
    ...
    // XS offsets
    for (let i = 0; i < numX; i++) {
      view.setUint32(baseOffset + i * 4, iter * xsElemStrides[i], true);
    }
    
    // YS offsets
    for (let i = 0; i < numY; i++) {
      view.setUint32(baseOffset + (numX + i) * 4, iter * ysElemStrides[i], true);
    }
  }
