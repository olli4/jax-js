...
async function createBackend(device) {
	if (device === "cpu") return new CpuBackend();
		...
		const { WebGPUBackend } = await Promise.resolve().then(() => require("./webgpu-f8JG43QJ.cjs"));
		...
		const { WebGLBackend } = await Promise.resolve().then(() => require("./webgl-Dg0H0ViR.cjs"));
...
Object.defineProperty(exports, 'normalizeIndexSpec', {
  enumerable: true,
  get: function () {
