...
//#endregion
const require_backend = require('./backend-BAV-024D.cjs');
const require_scan_wrapper = require('./scan-wrapper-DBgXYE11.cjs');
	...
	pprint() {
		const steps = this.steps.map((step) => {
			switch (step.type) {
				...
				case "copy": return require_backend.PPrint.pp(`copy %${step.dst} + %${step.src} -> %${step.output} (${step.totalBytes}B, ${step.updates.length} update${step.updates.length !== 1 ? "s" : ""})`);
...
function jitCompile(backend, jaxpr) {
	...
	for (let i = 0; i < jaxpr.eqns.length; i++) {
		...
		if (eqn.primitive === Primitive.DynamicUpdateSlice) {
			...
			const elemBytes = require_backend.byteWidth(dstAval.dtype);
			const totalBytes = dstAval.size * elemBytes;
			let updates;
			if (dstAval.shape.length === srcAval.shape.length) {
				const innerBefore = require_backend.prod(dstAval.shape.slice(axis + 1));
				const outerBefore = require_backend.prod(dstAval.shape.slice(0, axis));
