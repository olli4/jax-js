...
* ```
*/
function setScanPathCallback(callback) {
	scanPathCallback = callback;
}
/** Internal: report scan path choice to registered callback. */
function reportScanPath(path, backend, details) {
	if (scanPathCallback) scanPathCallback(path, backend, details);
}
...
var Kernel = class Kernel {
	outputs;
	constructor(nargs, outputs) {
		this.nargs = nargs;
		const outputArray = Array.isArray(outputs) ? outputs : [outputs];
		if (outputArray.length === 0) throw new Error("Kernel requires at least one output");
	...
	static single(nargs, size, exp, reduction) {
		return new Kernel(nargs, [{
			size,
			exp,
			reduction
		}]);
	...
	get hasReduction() {
		return this.outputs.some((o) => o.reduction !== void 0);
	}
	...
	get dtype() {
		...
		if (out.reduction) return out.reduction.epilogue.dtype;
		else return out.exp.dtype;
	}
	...
	get totalBytes() {
		return this.outputs.reduce((sum, out, i) => sum + out.size * byteWidth(this.dtypeAt(i)), 0);
	}
	...
	hash(state) {
		...
		for (const out of this.outputs) state.update(out.size).update(out.exp).update(out.reduction);
	}
	pprint() {
		if (this.outputs.length === 1) {
			let details = PPrint.pp(`exp = ${this.outputs[0].exp}`);
			details = details.concat(PPrint.pp(`size = ${this.outputs[0].size}`));
			if (this.outputs[0].reduction) details = details.concat(PPrint.pp(`reduction = ${this.outputs[0].reduction}`));
			...
			for (let i = 0; i < this.outputs.length; i++) {
				...
				if (out.reduction) outStr += `, reduction=${out.reduction}`;
				outStr += " }";
...
var Reduction = class Reduction {
	...
	reindexGids(newGids) {
		...
		if (reindexedEpilogue === this.epilogue) return this;
...
var CpuBackend = class {
	...
	dispatch(exe, inputs, outputs) {
		...
		if (kernel.isMultiOutput) return this.#dispatchMultiKernel(kernel, inputs, outputs);
		const { exp, epilogue } = tuneNullopt(kernel);
		const inputBuffers = inputs.map((slot) => this.#getBuffer(slot));
		const outputBuffers = outputs.map((slot) => this.#getBuffer(slot));
		const usedArgs = new Map([...exp.collect((exp$1) => exp$1.op === AluOp.GlobalIndex || exp$1.op === AluOp.GlobalView), ...epilogue ? epilogue.collect((exp$1) => exp$1.op === AluOp.GlobalIndex || exp$1.op === AluOp.GlobalView) : []].map((exp$1) => [exp$1.arg[0], exp$1.dtype]));
	...
	#dispatchMultiKernel(kernel, inputs, outputs) {
		const inputBuffers = inputs.map((slot) => this.#getBuffer(slot));
		const size = kernel.outputs[0].size;
		const allUsedArgs = /* @__PURE__ */ new Map();
		for (const out of kernel.outputs) {
			const usedArgs = out.exp.collect((exp) => exp.op === AluOp.GlobalIndex || exp.op === AluOp.GlobalView);
			for (const exp of usedArgs) allUsedArgs.set(exp.arg[0], exp.dtype);
		}
		const inputArrays = inputBuffers.map((buf, i) => {
		...
		const outputArrays = outputs.map((slot, i) => dtypedArray(kernel.dtypeAt(i), this.#getBuffer(slot)));
		const globals = (gid, bufidx) => {
			if (gid < 0 || gid >= inputArrays.length) throw new Error("gid out of bounds: " + gid);
			if (bufidx < 0 || bufidx >= inputArrays[gid].length) throw new Error("bufidx out of bounds: " + bufidx);
			return inputArrays[gid][bufidx];
		};
		for (let i = 0; i < size; i++) for (let outIdx = 0; outIdx < kernel.outputs.length; outIdx++) {
...
//#endregion
//#region src/backend/wasm/generated/routines.ts
/**
...
const ARGSORT_WASM = "";
/** cholesky routine compiled from src/routines/cholesky.ts */
const CHOLESKY_WASM = "";
/** lu routine compiled from src/routines/lu.ts */
const LU_WASM = "";
/** sort routine compiled from src/routines/sort.ts */
const SORT_WASM = "";
/** triangular-solve routine compiled from src/routines/triangular-solve.ts */
const TRIANGULAR_SOLVE_WASM = "";
...
const moduleCache$1 = /* @__PURE__ */ new Map();
/**
* Get a compiled WebAssembly module synchronously.
* Useful when async is not available.
*/
function getRoutineModuleSync(name) {
	if (moduleCache$1.has(name)) return moduleCache$1.get(name);
	let wasm;
	switch (name) {
		case "argsort":
			wasm = ARGSORT_WASM;
		...
		case "triangular-solve":
			...
			break;
		default: throw new Error(`Unknown routine: ${name}`);
	}
	const bytes = base64ToBytes(wasm);
	const module = new WebAssembly.Module(bytes);
	moduleCache$1.set(name, module);
	return module;
}
...
var WasmBackend = class {
	...
	#routineInstances = /* @__PURE__ */ new Map();
	constructor() {
	...
	#dispatchCholesky(routine, inputs, outputs, elementSize) {
		...
		const exportName = elementSize === 4 ? "cholesky_batched_f32" : "cholesky_batched_f64";
		const func = instance.exports[exportName];
		const shape = routine.type.inputShapes[0];
		func(this.#buffers.get(inputs[0]).ptr, this.#buffers.get(outputs[0]).ptr, shape[shape.length - 1], shape.slice(0, -2).reduce((a, b) => a * b, 1));
	}
	#dispatchTriangularSolve(routine, inputs, outputs, elementSize) {
		...
		const exportName = elementSize === 4 ? "triangular_solve_batched_f32" : "triangular_solve_batched_f64";
		const func = instance.exports[exportName];
		...
		func(this.#buffers.get(inputs[0]).ptr, this.#buffers.get(inputs[1]).ptr, this.#buffers.get(outputs[0]).ptr, n, batchRows, numBatches, routine.params?.unitDiagonal ? 1 : 0, routine.params?.lower ? 1 : 0);
	}
	#dispatchLU(routine, inputs, outputs, elementSize) {
		...
		const func = instance.exports[exportName];
		const shape = routine.type.inputShapes[0];
		func(this.#buffers.get(inputs[0]).ptr, this.#buffers.get(outputs[0]).ptr, this.#buffers.get(outputs[1]).ptr, this.#buffers.get(outputs[2]).ptr, shape[shape.length - 2], shape[shape.length - 1], shape.slice(0, -2).reduce((a, b) => a * b, 1));
	}
	#dispatchSort(routine, inputs, outputs, elementSize) {
		...
		const exportName = elementSize === 4 ? "sort_batched_f32" : "sort_batched_f64";
		const func = instance.exports[exportName];
	...
	#dispatchArgsort(routine, inputs, outputs, elementSize) {
		...
		const exportName = elementSize === 4 ? "argsort_batched_f32" : "argsort_batched_f64";
		const func = instance.exports[exportName];
		func(this.#buffers.get(inputs[0]).ptr, this.#buffers.get(outputs[0]).ptr, this.#buffers.get(outputs[1]).ptr, auxPtr, n, batchSize);
		this.#allocator.free(auxPtr);
	}
	/**
	...
	*/
	dispatchNativeScanGeneral(exe, params, consts, initCarry, xs, carryOut, ysStacked) {
		const internalSlots = [];
		...
		let auxPtr = 0;
		if (params.auxBufferSize && params.auxBufferSize > 0) auxPtr = this.#allocator.malloc(params.auxBufferSize);
		try {
			let instance = this.#instanceCache.get(exe.data.module);
			if (!instance) {
			...
			const func = instance.exports.scan;
			const ptrs = [
			...
			for (const slot of internalSlots) this.decRef(slot);
...
function translateExpCore(cg, funcs, exp, ctx) {
	...
	const gen = (e) => {
		...
		} else if (op === AluOp.Const) dty(cg, op, dtype).const(arg);
		else if (op === AluOp.Special) {
			const name = arg[0];
			const local = ctx.getVariable(name);
			if (local === void 0) throw new Error(`Unknown special: ${name}`);
			cg.local.get(local);
		} else if (op === AluOp.Variable) {
			const name = arg;
			const local = ctx.getVariable(name);
			if (local === void 0) throw new Error(`Unknown variable: ${name}`);
			cg.local.get(local);
		} else throw new UnsupportedOpError(op, dtype, "wasm");
...
*/
function codegenWasmKernel(kernel) {
	const isMultiOutput = kernel.isMultiOutput;
	if (isMultiOutput) return codegenWasmMultiPath(kernel);
	else return codegenWasmSinglePath(kernel);
}
/** Single-output kernel codegen (supports reduction). */
function codegenWasmSinglePath(kernel) {
...
*/
function tuneNulloptExp(exp, size) {
...
*/
function codegenWasmMultiPath(kernel) {
	const cg = new CodeGenerator();
	cg.memory.import("env", "memory");
	const size = kernel.outputs[0].size;
	...
	const kernelFunc = cg.function(rep(kernel.nargs + numOutputs, cg.i32), [], () => {
		const gidx = cg.local.declare(cg.i32);
		cg.loop(cg.void);
		cg.block(cg.void);
		cg.local.get(gidx);
		...
		for (let outIdx = 0; outIdx < numOutputs; outIdx++) {
			...
			cg.local.get(kernel.nargs + outIdx);
			cg.local.get(gidx);
...
function codegenNativeScanGeneral(params) {
	...
	if (DEBUG >= 2) console.log("codegenNativeScanGeneral params:", {
		length,
		numConsts,
		numCarry,
		numX,
		...
		for (let stepIdx = 0; stepIdx < steps.length; stepIdx++) {
			...
			if (step.source instanceof Routine) {
				...
				if (routineType === Routines.Cholesky) {
					...
					for (const param of callInfo.staticParams) cg.i32.const(param);
				...
				cg.loop(cg.void);
				{
					...
					for (let outIdx = 0; outIdx < tunedOutputs.length; outIdx++) {
						const out = tunedOutputs[outIdx];
						const internalIdx$1 = outIndices[outIdx];
						cg.local.get(internalsBase + internalIdx$1);
						cg.local.get(gidx);
						cg.i32.const(byteWidth(out.dtype));
						cg.i32.mul();
						cg.i32.add();
...
async function createBackend(device) {
	if (device === "cpu") return new CpuBackend();
		...
		const { WebGPUBackend } = await import("./webgpu-NPzHuFIq.js");
		...
		const { WebGLBackend } = await import("./webgl-yQcW5MZh.js");
...
//#endregion
export { AluExp, AluGroup, AluOp, AluVar, DEBUG, DType, Executable, FpHash, Kernel, PPrint, Reduction, Routine, Routines, ShapeTracker, SlotError, UnsupportedOpError, UnsupportedRoutineError, accessorAluExp, accessorGlobal, assertNonNull, byteWidth, checkAxis, checkInts, deepEqual, defaultDevice, devices, dtypedArray, dtypedJsArray, findPow2, generalBroadcast, getBackend, init, invertPermutation, isFloatDtype, isNumberPair, isPermutation, mapSetUnion, normalizeAxis, partitionList, prod, promoteTypes, range, recursiveFlatten, rep, reportScanBodySteps, reportScanPath, runWithCache, setDebug, setScanBodyStepsCallback, setScanPathCallback, strip1, toposort, tuneNullopt, tuneWebgpu, unravelAlu, unzip2, zip, zipn };