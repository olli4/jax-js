...
function normalizeIndexSpec(spec, name = "indices") {
	let indices;
	if (typeof spec === "number") indices = [spec];
	else if (Array.isArray(spec)) indices = [...spec];
	else {
		indices = [];
		for (const [key, enabled] of Object.entries(spec)) {
			if (!enabled) continue;
			if (!/^-?\d+$/.test(key)) throw new TypeError(`${name} dictionary keys must be integer strings, got ${key}`);
...
async function createBackend(device) {
	if (device === "cpu") return new CpuBackend();
		...
		const { WebGPUBackend } = await import("./webgpu-CMSuydbD.js");
		...
		const { WebGLBackend } = await import("./webgl-CJNHx3sh.js");
...
//#endregion
export { AluExp, AluGroup, AluOp, AluVar, DEBUG, DType, Executable, FpHash, Kernel, PPrint, Reduction, Routine, Routines, ShapeTracker, SlotError, UnsupportedOpError, UnsupportedRoutineError, accessorAluExp, accessorGlobal, assertNonNull, byteWidth, checkAxis, deepEqual, defaultDevice, devices, dtypedArray, dtypedJsArray, findPow2, generalBroadcast, getBackend, getScanRoutineInfo, init, invertPermutation, isFloatDtype, isNumberPair, isPermutation, mapSetUnion, normalizeAxis, normalizeIndexSpec, partitionList, prod, promoteTypes, range, recursiveFlatten, rep, runWithCache, setDebug, strip1, toposort, tuneNullopt, tuneWebgpu, unravelAlu, unzip2, zip, zipn };