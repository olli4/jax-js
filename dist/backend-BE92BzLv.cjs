...
function getRoutineModuleSync(name) {
	...
	moduleCache$1.set(name, module$1);
	return module$1;
}
...
var WasmBackend = class {
	...
	#getRoutineInstance(name) {
		...
		if (!instance) {
			const module$1 = getRoutineModuleSync(moduleName);
...
async function createBackend(device) {
	if (device === "cpu") return new CpuBackend();
		...
		const { WebGPUBackend } = await Promise.resolve().then(() => require("./webgpu-D-atFHoA.cjs"));
		...
		const { WebGLBackend } = await Promise.resolve().then(() => require("./webgl-BDpV2ach.cjs"));
...
Object.defineProperty(exports, 'setScanPathCallback', {
  enumerable: true,
  get: function () {
    return setScanPathCallback;
  }
});
