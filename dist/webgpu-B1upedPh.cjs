...
var WebGPUBackend = class WebGPUBackend {
	...
	*/
	...
	#poolBudgetBytes = 64 * 1024 * 1024;
	#poolCurrentBytes = 0;
	static MAX_POOL_PER_SIZE = 4;
	/** Fallback budget for eager mode (no JitProgram to derive peak from). */
	static MAX_POOL_BYTES_DEFAULT = 64 * 1024 * 1024;
	/**
	...
	*/
	...
	malloc(size, initialData) {
		...
		} else buffer = this.#poolPop(paddedSize) ?? this.#createBuffer(paddedSize);
	...
	*/
	preparePreencodedScan(params) {
		...
		if (numX === 0 && numY === 0) {
			if (require_backend.DEBUG >= 2) console.log("Preencoded scan: no xs/ys, skipping");
			return null;
		}
		const scanInfo = {
			numConsts,
	...
	*/
	...
	#poolPush(buffer) {
		...
		if (this.#poolCurrentBytes + paddedSize > this.#poolBudgetBytes) return false;
		list.push(buffer);
	...
	*/
	configurePool(hints) {
		for (const [size, list] of this.#bufferPool) if (!hints.mallocSizes.has(size)) {
		...
		while (this.#poolCurrentBytes > this.#poolBudgetBytes) {
			let evicted = false;
			for (const [size, list] of this.#bufferPool) if (list.length > 0) {
				const buf = list.pop();
				this.#poolCurrentBytes -= buf.size;
				this.#gpuAllocatedBytes -= buf.size;
				buf.destroy();
	...
	#createBuffer(size, { mapped = false, read = false } = {}) {
		...
		if (!read) this.#gpuAllocatedBytes += size;
		return buffer;
	}
};
/**
	...
	const gen = (e) => {
		...
		if (op === require_backend.AluOp.GlobalIndex) {
			...
			if (gid < numConsts) {
				...
				return eDtype === require_backend.DType.Bool ? `(${access} != 0)` : access;
		...
		if (op === require_backend.AluOp.Erf || op === require_backend.AluOp.Erfc) {
			const funcName = op === require_backend.AluOp.Erf ? "erf" : "erfc";
			const a = require_backend.strip1(gen(src[0]));
			if (eDtype !== require_backend.DType.Float32) return `${dtypeToWgsl(eDtype)}(${funcName}(f32(${a})))`;
			return `${funcName}(${a})`;
		}
		if (require_backend.AluGroup.Binary.has(op) || require_backend.AluGroup.Compare.has(op)) {
			const a = gen(src[0]);
			const b = gen(src[1]);
			if (op === require_backend.AluOp.Add) return eDtype === require_backend.DType.Bool ? `(${a} || ${b})` : `(${a} + ${b})`;
			if (op === require_backend.AluOp.Sub) return `(${a} - ${b})`;
			if (op === require_backend.AluOp.Mul) return eDtype === require_backend.DType.Bool ? `(${a} && ${b})` : `(${a} * ${b})`;
			if (op === require_backend.AluOp.Idiv) return require_backend.isFloatDtype(eDtype) ? `trunc(${a} / ${b})` : `(${a} / ${b})`;
			if (op === require_backend.AluOp.Mod) return `(${a} % ${b})`;
			if (op === require_backend.AluOp.Min) return eDtype === require_backend.DType.Bool ? `(${a} && ${b})` : `min(${require_backend.strip1(a)}, ${require_backend.strip1(b)})`;
			if (op === require_backend.AluOp.Max) return eDtype === require_backend.DType.Bool ? `(${a} || ${b})` : `max(${require_backend.strip1(a)}, ${require_backend.strip1(b)})`;
			if (op === require_backend.AluOp.Cmplt) return `(${a} < ${b})`;
			if (op === require_backend.AluOp.Cmpne) return `(${a} != ${b})`;
		}
		if (require_backend.AluGroup.Unary.has(op)) {
			...
			if (op === require_backend.AluOp.Cast) return `${dtypeToWgsl(eDtype)}(${require_backend.strip1(a)})`;
			if (op === require_backend.AluOp.Bitcast) return `bitcast<${dtypeToWgsl(eDtype)}>(${require_backend.strip1(a)})`;
		}
		if (op === require_backend.AluOp.Where) return `select(${require_backend.strip1(gen(src[2]))}, ${require_backend.strip1(gen(src[1]))}, ${require_backend.strip1(gen(src[0]))})`;
