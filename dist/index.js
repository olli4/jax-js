...
function tryPrepareWebGPUNativeScan(backend, bodyProgram, bodyJaxpr, executeSteps, length, numCarry, numConsts, numX, numY, reverse) {
	...
	if (executeSteps.length === 1 && numCarry === 1 && numY === 1) {
		...
		try {
			...
			if (exe) {
				if (DEBUG >= 1) console.log(`[webgpu-scan] SUCCESS! Using WebGPU native scan (single kernel)`);
				return {
					executable: exe,
					internalSizes: []
				};
	...
	try {
		...
		if (exe) {
			if (DEBUG >= 1) console.log(`[webgpu-scan] SUCCESS! Using WebGPU native scan (${multiSteps.length} kernels)`);
			return {
				executable: exe,
				internalSizes: []
			};
		...
		return {
			...
			[Primitive.Jit](args, { jaxpr }) {
				...
				const scanRunner = (bodyProgram, _backend, bodyJaxpr, length, numCarry, _numConsts, _numX, numY, reverse, constSlots, initCarrySlots, xsSlots, xsAvals, _outputSlots) => {
					...
					const xSliceSts = xs.map((x) => {
						const sliceSts = [];
						const squeezedShape = x.shape.slice(1);
						for (let iter = 0; iter < length; iter++) {
							const i = reverse ? length - 1 - iter : iter;
							const slicePairs = x.shape.map((s, axis) => axis === 0 ? [i, i + 1] : [0, s]);
