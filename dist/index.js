...
function tril(a, k = 0) {
	...
	return where$1(tri(n, m, k, { dtype: DType.Bool }), a, zerosLike$1(a));
}
...
function triu(a, k = 0) {
	...
	a = fudgeArray(a);
	const [n, m] = a.shape.slice(-2);
	return where$1(tri(n, m, k - 1, { dtype: DType.Bool }), zerosLike$1(a), a);
}
/**
...
function isinf(x) {
	x = fudgeArray(x);
	return isFloatDtype(x.dtype) ? x.equal(Infinity).add(x.equal(-Infinity)) : fullLike$1(x, false);
}
