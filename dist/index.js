...
function tryPrepareWebGPUNativeScan(backend, bodyProgram, bodyJaxpr, executeSteps, length, numCarry, numConsts, numX, numY, reverse) {
	if (DEBUG >= 2) console.log(`[webgpu-scan] trying with numCarry=${numCarry}, numY=${numY}, steps=${executeSteps.length}`);
	...
	if (executeSteps.length === 1 && numCarry === 1 && numY === 1) {
		...
		for (const inputId of step.inputs) if (inputId < numInputs) reindexMap.push(inputId);
			...
			if (DEBUG >= 2) console.log("[webgpu-scan] single kernel has internal buffer ref");
			return null;
		}
	...
	if (numCarry !== numY && numY !== 0) {
		if (DEBUG >= 2) console.log(`[webgpu-scan] multi-kernel requires numCarry === numY or numY === 0, got ${numCarry} !== ${numY}`);
		return null;
	}
	const hasInternalDeps = executeSteps.some((step) => step.inputs.some((inputId) => inputId >= numInputs));
	if (hasInternalDeps) {
		if (DEBUG >= 2) console.log("[webgpu-scan] multi-kernel: internal buffer dependencies not supported, falling back");
		return null;
	}
	...
	for (let carryIdx = 0; carryIdx < numCarry; carryIdx++) {
		...
		if (passthroughIdx !== -1) {
			if (DEBUG >= 2) console.log(`[webgpu-scan] multi-kernel: carry ${carryIdx} is passthrough, not supported`);
			return null;
		}
		const source = slotToSource.get(slot);
		if (!source) {
			if (DEBUG >= 2) console.log(`[webgpu-scan] multi-kernel: carry output ${carryIdx} (slot ${slot}) not produced by any step`);
			return null;
		}
	...
	for (const { source, carryIdx } of carrySourceInfos) {
		...
		for (const inputId of step.inputs) if (inputId < numInputs) inputs.push(inputId);
			...
			if (outIdx !== void 0) inputs.push(numInputs + outIdx);
				...
				if (DEBUG >= 2) console.log(`[webgpu-scan] multi-kernel: input ${inputId} not mapped`);
				return null;
			}
		}
		let reindexedKernel;
		if (stepSource instanceof Kernel) {
			...
			reindexedKernel = Kernel.single(numInputs + totalOutputs, output.size, reindexedExp, reindexedReduction);
		} else {
			if (DEBUG >= 2) console.log("[webgpu-scan] multi-kernel: unexpected source type at step");
			return null;
		}
		multiSteps.push({
			kernel: reindexedKernel,
	...
	try {
		...
		if (exe && DEBUG >= 1) console.log(`[webgpu-scan] SUCCESS! Using WebGPU native scan (${multiSteps.length} kernels)`);
		return exe ? { executable: exe } : null;
	} catch (e$1) {
		if (DEBUG >= 2) console.warn("[webgpu-scan] prepareNativeScanMulti failed:", e$1);
	}
...
function tryPrepareWasmNativeScan(backend, bodyProgram, bodyJaxpr, executeSteps, length, numCarry, numConsts, numX, numY, reverse) {
	...
	for (const step of executeSteps) if (step.source instanceof Routine) {
		...
		if (!supportedRoutines.has(routineName)) {
			if (DEBUG >= 1) console.log(`[wasm-scan] skipped, unsupported routine in scan body: ${Routines[routineName]}`);
			return null;
		}
	...
	for (let i = 0; i < executeSteps.length; i++) {
		...
		if (source instanceof Kernel) if (source.isMultiOutput) for (let outIdx = 0; outIdx < source.outputs.length; outIdx++) {
			const internalIdx = internalSizes.length;
			slotToInternal.set(step.outputs[outIdx], internalIdx);
			internalSizes.push(source.outputs[outIdx].size * byteWidth(source.dtypeAt(outIdx)));
		}
		else {
			const internalIdx = internalSizes.length;
	...
	for (let i = 0; i < executeSteps.length; i++) {
		...
		if (source instanceof Kernel) {
			...
			const reindexedKernel = Kernel.multi(numInputs + internalSizes.length, reindexedOutputs);
		...
		const steps = this.steps.map((step) => {
			switch (step.type) {
				case "execute": {
					...
					if (step.source instanceof Routine) return PPrint.pp(`${executeText}, routine ${step.source.name}`);
					else if (step.source.isMultiOutput) return PPrint.pp(`${executeText}, multi-kernel`).concat(step.source.pprint().indent(2));
					else return PPrint.pp(`${executeText}, kernel`).concat(step.source.pprint().indent(2));
				}
...
var JitProgramBuilder = class {
	...
	pushLit(lit) {
		const kernel = Kernel.single(0, lit.aval.size, AluExp.const(lit.dtype, lit.value));
	...
	for (let i = 0; i < jaxpr.eqns.length; i++) {
		...
		for (let i$1 = 0; i$1 < eqn.outBinders.length; i$1++) {
			...
			if (blackNodes.has(outVar)) {
				...
				if (reduction) {
					flushPendingKernels();
					const kernel = Kernel.single(nargs$1, size$1, exp$2[i$1], reduction);
...
var PendingExecute = class {
	...
	async prepare() {
		...
		this.#promise = (async () => {
			if (this.source instanceof Routine) this.prepared = await this.backend.prepareRoutine(this.source);
	...
	prepareSync() {
		if (this.prepared) return;
		if (this.source instanceof Routine) this.prepared = this.backend.prepareRoutineSync(this.source);
...
var Array$1 = class Array$1 extends Tracer {
	...
	*/
	#gather(indices, axis, outDim) {
		...
		const kernel = Kernel.single(inputs.length, prod(finalShape), exp$2);
	...
	#unary(op, dtypeOutput) {
		...
		const kernel = Kernel.single(1, this.#st.size, exp$2);
		const output = this.#backend.malloc(kernel.bytes);
		const pending = [...this.#pending];
		for (const exe of pending) exe.updateRc(1);
	...
	static #naryCustom(name, custom, arrays, { dtypeOverride, strongTypeOutput, reduceAxis } = {}) {
		...
		const kernel = Kernel.single(inputs.length, prod(newShape), exp$2, re);
	...
	#reduce(op) {
		...
		const kernel = Kernel.single(inputs.length, newSize, exp$2, reduction);
		const output = this.#backend.malloc(kernel.bytes);
		const pending = [...this.#pending];
		for (const exe of pending) exe.updateRc(1);
		pending.push(new PendingExecute(this.#backend, kernel, inputs, [output]));
	...
	*/
	#realize() {
		...
		if (this.#source instanceof AluExp) {
			...
			const kernel = Kernel.single(0, this.#st.size, exp$2);
			const output = this.#backend.malloc(kernel.bytes);
			...
			[Primitive.Jit](args, { jaxpr }) {
				...
				const scanRunner = ({ bodyProgram, backend: _backend, bodyJaxpr, length, numCarry, numConsts: _numConsts, numX: _numX, numY, reverse, constSlots, initCarrySlots, xsSlots, xsAvals, outputSlots }) => {
					...
					const runBody = (curCarry, xSlice, i) => {
						...
						if (DEBUG >= 2) console.log(`[scanRunner] iter ${i}: carrySlots=${carrySlots}, xSliceSlots=${xSliceSlots}`);
						const { outputs: bodyOuts, pending: pending$1 } = bodyProgram.execute([
							...constSlotsRealized,
							...carrySlots,
							...xSliceSlots
						]);
						if (DEBUG >= 2) console.log(`[scanRunner] iter ${i}: bodyOuts=${bodyOuts}, pending.length=${pending$1.length}`);
			...
			[Primitive.Scan](args, { jaxpr, numCarry, numConsts, length, reverse }) {
				...
				if (canDirectWriteY) {
					...
					for (const atom of yOutAtoms) {
						...
						const ySizeBytes = length * prod(aval.shape) * byteWidth(aval.dtype);
						if (ySizeBytes === 0) {
