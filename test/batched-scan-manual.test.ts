...
import { init, numpy as np, lax, jit } from "../src/index";
import type { WebGPUBackend, BatchedScanParams, PreparedBatchedScan } from "../src/backend/webgpu";

describe("batched scan manual test", () => {
  it("verifies uniform offset approach with matmul body", async () => {
    const backend = await init("webgpu");
    if (!backend || backend.type !== "webgpu") {
    ...
    console.log("Final carry (reference):", Array.from(finalCarryData).slice(0, 16));
  ...
  it("tests prepareBatchedScan shader wrapping", async () => {
    const backend = await init("webgpu") as WebGPUBackend | null;
    if (!backend || backend.type !== "webgpu") {
      console.log("WebGPU not available, skipping");
      return;
    }
...
`,
    ...
    const { buffer, alignment: actualAlignment } = createAllIterationsOffsetsBuffer(
      1, 1, length, xsElemStrides, ysElemStrides, alignment
    );
