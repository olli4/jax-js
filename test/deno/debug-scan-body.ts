...
const step = (carry: typeof np.Array.prototype, b: typeof np.Array.prototype): [typeof np.Array.prototype, typeof np.Array.prototype] => {
  // Lower triangular matrix L - simplified: identity + lower off-diagonal
  // [[1, 0], [0.5, 1]]
  const L = np.array([[1., 0.], [0.5, 1.]], "float32");
  // Solve L @ x = b for x
  const x = lax.linalg.triangularSolve(L, b.reshape([2, 1]), { leftSide: true, lower: true });
...
const jittedStep = jit((carry: typeof np.Array.prototype, b: typeof np.Array.prototype): [typeof np.Array.prototype, typeof np.Array.prototype] => {
  const L = np.array([[1., 0.], [0.5, 1.]], "float32");
  const x = lax.linalg.triangularSolve(L, b.reshape([2, 1]), { leftSide: true, lower: true });
  const xFlat = x.reshape([2]);
  return [xFlat.ref, xFlat];
