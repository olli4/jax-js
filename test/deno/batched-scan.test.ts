...
// Import scan-wrapper from source (these functions aren't exported in dist)
import {
  wrapRoutineForScan,
  createAllIterationsOffsetsBuffer,
} from "../../src/backend/webgpu/scan-wrapper.ts";

const hasWebGPU = typeof navigator !== "undefined" && "gpu" in navigator;

// ============================================================================
// Pure WebGPU tests (no jax-js) - run first to avoid device memory conflicts
// ============================================================================
...
Deno.test({
  ...
  fn: async () => {
    ...
    const { buffer, alignment: actualAlignment } = createAllIterationsOffsetsBuffer(
      1,
      1,
      length,
      xsElemStrides,
    ...
    console.log("✓ Offset buffer has correct element offsets for each iteration");
  },
});
...
`;
    ...
    const uniformBindGroup = device.createBindGroup({
      ...
      entries: [{ binding: 0, resource: { buffer: offsetBuffer, offset: 0, size: 8 } }],
    });
    ...
    commandEncoder.copyBufferToBuffer(finalCarryBuffer, 0, stagingBuffer, 0, carrySize);
    ...
    commandEncoder.copyBufferToBuffer(ysBuffer, 0, ysStagingBuffer, 0, ysSize);

    device.queue.submit([commandEncoder.finish()]);

    // Read results
    await stagingBuffer.mapAsync(GPUMapMode.READ);
    const finalCarryResult = new Float32Array(stagingBuffer.getMappedRange().slice(0));
    stagingBuffer.unmap();

    await ysStagingBuffer.mapAsync(GPUMapMode.READ);
    const ysResult = new Float32Array(ysStagingBuffer.getMappedRange().slice(0));
    ...
    console.log("✓ End-to-end batched scan dispatch with uniform offsets works correctly!");
...
`,
    ...
    console.log("✓ Pipeline has expected bind group layouts (group 0: storage, group 1: uniform)");
...
// ============================================================================
// jax-js integration tests - run last to avoid device memory conflicts
// ============================================================================
...
Deno.test({
  ...
  fn: async () => {
    ...
    const { init, numpy: np, lax, defaultDevice } = await import("../../dist/index.js");
    ...
    console.log("Final carry (JS loop):", Array.from(finalCarryData).slice(0, 16));
