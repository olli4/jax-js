...
const G = np.array([[1.0, 1.0], [0.0, 1.0]], "float32");  // [2, 2] transition matrix
const W = np.array([[0.01, 0.0], [0.0, 0.01]], "float32");  // [2, 2] state noise
...
const forwardStep = (
  ...
  inp: { y: typeof np.Array.prototype; V2: typeof np.Array.prototype }
    ...
    { x_pred: typeof np.Array.prototype; C_pred: typeof np.Array.prototype; v: typeof np.Array.prototype; Cp: typeof np.Array.prototype }] => {
...
const C0 = np.array([[1000.0, 0.0], [0.0, 1000.0]], "float32");  // initial covariance
...
for (const n of [10, 50, 100]) {
  ...
  try {
    ...
    console.log(`       ${elapsed.toFixed(1).padStart(7)}ms (${(elapsed/n).toFixed(2)}ms/iter)`);
