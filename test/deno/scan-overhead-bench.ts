...
for (const n of [10, 50, 100, 200, 500, 1000]) {
  ...
  console.log(`n=${n.toString().padStart(4)}: ${elapsed.toFixed(1).padStart(7)}ms (${(elapsed/n).toFixed(3)}ms/iter)`);
}
...
console.log(`Linear fit: time = ${fixedMs.toFixed(1)}ms + ${perIterMs.toFixed(3)}ms × n`);
...
console.log(`With batched scan (theoretical): ${batchedOverhead.toFixed(1)}ms + compute time`);
console.log(`Potential speedup: ~${(estimatedKalman / batchedOverhead).toFixed(0)}×`);
