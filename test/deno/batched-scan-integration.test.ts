...
Deno.test("sort-in-scan works on WebGPU (falls back to JS loop due to Sort using uniforms)", async () => {
  ...
  const step = (carry: typeof np.Array.prototype, x: typeof np.Array.prototype): [typeof np.Array.prototype, typeof np.Array.prototype] => {
