...
if [ "$SKIPPED_COUNT" -gt "0" ]; then
    ...
    if command -v "$HOME/.deno/bin/deno" &> /dev/null || command -v deno &> /dev/null; then
        pnpm run test:deno || {
            echo "ERROR: Deno WebGPU tests failed"
            exit 1
        }
        echo ""
        echo "OK: All tests passed (Vitest + Deno WebGPU)"
    else
        echo "WARN: Deno not found, skipping WebGPU tests"
        echo "   Install Deno: curl -fsSL https://deno.land/install.sh | sh"
        echo ""
        echo "OK: Vitest tests passed (WebGPU tests skipped)"
    fi
else
    echo ""
    echo "OK: All tests passed (WebGPU available in Chromium)"
fi
